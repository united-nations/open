...
export function BudgetSubviewNavigation({ basePath, label, contributorHash }: {
  basePath: string;
  label: string;
  ...
  useEffect(() => {
    const followLegacyLink = () => {
      ...
      if (pathname.replace(/\/$/, "") === basePath &&
        (hash === `${contributorHash}s` || hash.startsWith(`${contributorHash}=`))) {
  ...
  return <SectionSubnav nested label={label} items={[
    { href: basePath, label: "Budget" },
